...
# unanswered user turn (the probe — a multi-part request). This makes exactly
# ONE real call to the agent, on that final turn, with everything before it
# passed in as-is — mirroring exactly how production replays history
# (backend/main.py's _history_to_messages -> _initial_state -> a single
# ainvoke per new user message), rather than having the eval itself simulate
# every prior turn live. What's being measured is whether that one real
# response fully addresses every part of the probe request.
